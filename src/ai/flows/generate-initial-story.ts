...
/**
 * @fileOverview Ce fichier définit le flux Genkit pour générer l'histoire initiale basée sur le thème choisi, le prompt du sous-thème et le nom du joueur. Il génère également un prompt d'image initial.
 *
...
const GenerateInitialStoryInputSchema = z.object({
  ...
  subThemePrompt: z.string().describe("Le prompt de scénario spécifique pour le sous-thème choisi par le joueur."), // Added subThemePrompt
...
const GenerateInitialStoryOutputSchema = z.object({
  story: z.string().describe("Le contenu initial de l'histoire basé sur le thème et le sous-thème choisi, s'adressant au joueur par son nom."),
  choices: z
    .array(z.string())
...
const prompt = ai.definePrompt({
  ...
  input: {
    schema: z.object({
      ...
      subThemePrompt: z.string().describe("Le prompt de scénario spécifique pour le sous-thème choisi."), // Added subThemePrompt schema
      playerName: z.string().describe('Le nom du joueur.'),
    }),
  },
  output: {
  ...
  prompt: `Tu es un Maître du Jeu (MJ) / Narrateur sympathique, créatif et plein d'humour pour un jeu d'aventure textuel interactif destiné aux enfants de 8 à 12 ans. Le nom du joueur est {{{playerName}}}. Ta mission est de démarrer une histoire passionnante et immersive basée sur le thème principal et le **scénario de départ spécifique (sous-thème)** choisi par le joueur, en t'adressant à lui par son nom, en définissant un **lieu de départ clair**, en proposant des choix et en générant un **prompt d'image initial**, tout en respectant les règles ci-dessous.

**Contexte de l'aventure :**
* Thème Principal : {{{theme}}}
* **Scénario de Départ (Sous-Thème)** : {{{subThemePrompt}}}  <-- **UTILISE CE SCÉNARIO SPÉCIFIQUE comme point de départ pour l'histoire initiale.**
...
3. **Le début de l'histoire (basé sur le sous-thème)** :
    * Commence directement l'histoire **en te basant sur le SCÉNARIO DE DÉPART ({{{subThemePrompt}}})**. Pas d'introduction générique.
...
4. **Le prompt d'image initial** : Génère un prompt CONCIS et DESCRIPTIF pour une image représentant la scène de départ que tu viens de décrire (basée sur le sous-thème). Ce prompt DOIT inclure une mention du **thème principal ({{{theme}}})**, du **nom du lieu ('location')**, et spécifier un **style cartoon**. Remplis la clé 'generatedImagePrompt' avec ce prompt. Si la description est très simple, tu peux laisser le prompt vide, mais essaie d'en générer un.
5. **Les premiers choix** : Propose 2 à 4 actions créatives, intéressantes et logiques que {{{playerName}}} peut choisir pour commencer l'aventure, **découlant directement de la situation de départ du sous-thème**. Ces choix doivent être pertinents pour le thème et le scénario, mais peuvent aussi surprendre le joueur. Formate les choix comme un tableau (array) de chaînes de caractères (strings).
...
Exemple de sortie attendue (pour thème: Fantasy Médiévale, sous-thème prompt: "Tu commences dans une vieille bibliothèque...", joueur: Alex) :
{
...
Génère maintenant l'histoire de départ, le lieu de départ ('location'), les premiers choix, et le prompt d'image initial ('generatedImagePrompt') en te basant **SPÉCIFIQUEMENT sur le scénario de départ suivant : '{{{subThemePrompt}}}'**, pour le thème principal **{{{theme}}}**, en t'adressant au joueur **{{{playerName}}}**, et en suivant TOUTES les règles indiquées. N'hésite pas à être créatif, à poser des questions, à ajouter du mystère, et de l'humour.
`,
});
...
const generateInitialStoryFlow = ai.defineFlow<typeof GenerateInitialStoryInputSchema, typeof GenerateInitialStoryOutputSchema>(
  ...
  async input => {
    // Input validation
    if (!input.theme || !input.subThemePrompt || !input.playerName) {
      throw new Error("Theme, subThemePrompt, and playerName are required for initial story generation.");
    }
