...
const GenerateStoryContentOutputSchema = z.object({
  ...
  generatedImagePrompt: z.string().optional().describe("Un prompt concis et descriptif pour la génération d'images UNIQUEMENT si une scène visuellement distincte se produit. DOIT viser la cohérence avec les images précédentes (si `previousImagePrompt` existe), notamment l'apparence du personnage (incluant nom et classe) et le style. Inclure le thème, le lieu actuel et spécifier \"Style : Cartoon\". Laisser vide sinon."), // Updated description
});
...
8.  **GÉNÉRATION D'IMAGE PROMPT (Consistance & Pertinence)** :
    ...
    *   **Exemples (avec consistance obligatoire)**:
        *   (Tour N) Prompt Précédent: "Le chevalier souriant JoueurTest avec une armure bleue découvrant une épée lumineuse dans une grotte sombre (lieu: Grotte aux Échos). Thème: Fantasy Médiévale. Style: Cartoon. Ambiance mystérieuse."
        *   (Tour N+1, si action = prendre épée) Nouveau Prompt: "Le chevalier souriant JoueurTest (Guerrier) dans son armure bleue brandissant fièrement l'épée lumineuse, qui éclaire vivement la Grotte aux Échos (lieu: Grotte aux Échos). Thème: Fantasy Médiévale. Style: Cartoon." (Conserve 'chevalier souriant', 'armure bleue', 'épée lumineuse', 'Grotte aux Échos', thème, style, ajoute classe, nom).
        *   (Tour N+2, si action = sortir de la grotte) Nouveau Prompt: "Le chevalier souriant JoueurTest (Guerrier) en armure bleue, portant l'épée lumineuse à sa ceinture, sortant de la Grotte aux Échos pour arriver dans une clairière ensoleillée et fleurie (lieu: Clairière Ensoleillée). Thème: Fantasy Médiévale. Style: Cartoon." (Conserve 'chevalier souriant', 'armure bleue', mentionne l'épée, nouveau lieu, thème, style, ajoute classe, nom).
        *   (Tour M) Prompt Précédent: "L'astronaute prudent JoueurAstro (Magicien) avec un casque rouge flottant devant une nébuleuse violette (lieu: Ceinture d'Astéroïdes Delta). Thème: Exploration Spatiale. Style: Cartoon. Air émerveillé."
        *   (Tour M+1, si action = examiner vaisseau) Nouveau Prompt: "" (Pas d'image car action peu visuelle).
        *   (Tour M+2, si action = rencontrer alien amical) Nouveau Prompt: "L'astronaute prudent JoueurAstro (Magicien) avec son casque rouge, souriant, flottant à côté d'un petit alien vert aux grands yeux dans une station spatiale lumineuse (lieu: Station Alpha). Thème: Exploration Spatiale. Style: Cartoon." (Conserve 'astronaute prudent', 'casque rouge', thème, style, nouveau lieu, ajoute alien, ajoute classe, nom).
